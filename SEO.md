# Migration du site vers `lagrandecroisade.fr`

Guide opérationnel pour faire passer le site
`jean-desaintangel.github.io/horus_heresy/` sur le nom de domaine
**`lagrandecroisade.fr`** (achat prévu le **mercredi 29 juillet 2026**), tout en
...
URL canonique retenue : **l'apex `https://lagrandecroisade.fr/`** (le `www`
redirigera vers lui).
...
## 2. Phase B — acheter `lagrandecroisade.fr` (mercredi)
...
### Apex (`lagrandecroisade.fr`) — 4 enregistrements A
...
_(À revérifier sur la doc GitHub le jour J — ces IP peuvent évoluer.)_
...
`www.lagrandecroisade.fr` → `lagrandecroisade.fr` (puisque c'est l'apex qui sera
déclaré comme domaine personnalisé).

### Ne crée PAS

- d'enregistrement joker `*.lagrandecroisade.fr` → risque de prise de contrôle
  de sous-domaine ;
...
Resolve-DnsName lagrandecroisade.fr -Type A
Resolve-DnsName www.lagrandecroisade.fr -Type CNAME

# Sous Linux/macOS/WSL
dig lagrandecroisade.fr +noall +answer
dig www.lagrandecroisade.fr +noall +answer
```
...
2. Section _Custom domain_ : saisir `lagrandecroisade.fr` → **Save**.
...
Profil GitHub → **Settings** → **Pages** → _Add a domain_. GitHub donne un
...
- Symptôme fréquent pendant la transition : erreur de certificat sur
  `www.lagrandecroisade.fr`. Normal, GitHub émet un certificat par nom ; attends.

À partir de là, **GitHub redirige automatiquement (301)**
`jean-desaintangel.github.io/horus_heresy/*` vers `lagrandecroisade.fr/*`.
...
| xargs sed -i 's|https://jean-desaintangel\.github\.io/horus_heresy|https://lagrandecroisade.fr|g'
```
...
Get-ChildItem -Recurse -Include *.html,*.md,*.js |
  ForEach-Object {
    (Get-Content $_ -Raw) -replace 'https://jean-desaintangel\.github\.io/horus_heresy','https://lagrandecroisade.fr' |
...
```html
<link rel="canonical" href="https://lagrandecroisade.fr/" />
```

Dans chaque `pages/xxx.html`, en adaptant le nom de fichier :

```html
<link rel="canonical" href="https://lagrandecroisade.fr/pages/regles.html" />
```
...
for f in pages/*.html; do
  echo "$f → https://lagrandecroisade.fr/$f"
done
```
...
Sitemap: https://lagrandecroisade.fr/sitemap.xml
```

_Pédagogique_ : `robots.txt` ne protège rien (il est public et purement
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>https://lagrandecroisade.fr/pages/assaut.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/defi.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/armes.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/statuts-reactions.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/regles.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/psy.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/vehicule.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/titan.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/choix-legion.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/telechargement.html</loc><lastmod>2026-07-29</lastmod></url>
  <url><loc>https://lagrandecroisade.fr/pages/contact.html</loc><lastmod>2026-07-29</lastmod></url>
</urlset>
```
...
<html lang="fr">
  ...
  <body>
    ...
    <main id="contenu">
      ...
      <p>
        L’adresse demandée n’existe pas (ou plus).
...
1. **Google Search Console** → _Ajouter une propriété_ → type **Domaine**
...
dig lagrandecroisade.fr +short

# 2. HTTPS valide et pas de chaîne de redirections
curl -sIL https://lagrandecroisade.fr/ | grep -i "^HTTP\|^location"

# 3. www redirige bien vers l'apex (301, une seule étape)
curl -sIL https://www.lagrandecroisade.fr/ | grep -i "^HTTP\|^location"
...
curl -s https://lagrandecroisade.fr/robots.txt
curl -sI https://lagrandecroisade.fr/sitemap.xml | head -1

# 6. La 404 personnalisée sort bien
curl -sI https://lagrandecroisade.fr/page-qui-n-existe-pas | head -1   # attendu : 404
```

Puis, dans le navigateur :

- **Lighthouse** (DevTools → Lighthouse) sur l'accueil et `regles.html` :
  onglets _SEO_ et _Accessibilité_. Vise 100 en SEO ; les points restants après
  cette migration seront surtout des détails de contraste.
- **Partage** : colle `https://lagrandecroisade.fr/pages/regles.html` dans un
  message Discord et vérifie que la vignette `hero.webp` s'affiche. Si Discord
  garde l'ancienne image en cache, utilise le _Facebook Sharing Debugger_ pour
  forcer un rafraîchissement.
- `site:lagrandecroisade.fr` dans Google, une fois par semaine, pour suivre le
...
1. Settings → Pages → vider le champ _Custom domain_ → **Save**.
